...
//Exercico 7:
/*
float comprimento, largura, area_total=0;
char comodo[20];

do{
    printf("Insira qual o comodo deseja calcular a area: \n");
    fgets(comodo, 20, stdin);
    fflush(stdin);
    if(strcmp(comodo,"FIM\n") == 0){
        break;
    }
    ...
    printf("Insira a larguta do comodo: \n");
    scanf("%f", &largura);
    fflush(stdin);

    area_total = area_total+ largura*comprimento;
}while(1);

    printf("A area residencial total eh: %.2f metros quadrados", area_total);
*/
    return 0;
